...
% Windows = [Channel0+30,Channel0+40,Channel0+230,Channel0+290];
% Windows = [Channel0+50,Channel0+85,Channel0+220,Channel0+280];
% Windows = [Channel0+13,Channel0+16,Channel0+17,Channel0+18];
 Windows = [Channel0+15,Channel0+19,Channel0+90,Channel0+100];
...
func= @(C,x)C(1)*(x-C(2)).^(-C(3))+C(4)*(x-C(2)).^(-C(5))+C(6);
C1 = [max(FitSpec)/2,    0,  2.5, max(FitSpec)/2, 3.5,       1000]; % Initial guesses for the fit parameters
lb = [             1, -0.01,    1,       0.000001,   2, -100000]; % Lower bounds for the fit parameters
ub = [           Inf,  0.01,   10,            Inf,  10,  100000]; % Upper bounds for the fit parameters 
...
fitfunctionwin(1,:) = ft(1)*(fitEN(1,:)-ft(2)).^(-ft(3))+ft(4)*(fitEN(1,:)-ft(2)).^(-ft(5))+ft(6);
...
fitfunction(1,:) = ft(1)*(EN(1,:)-ft(2)).^(-ft(3))+ft(4)*(EN(1,:)-ft(2)).^(-ft(5))+ft(6);
...
%Fit Linear Combination of 2 Power Laws to the peaks.
func= @(C,x)C(1)*(x-C(2)).^(-C(3))+C(4)*(x-C(2)).^(-C(5))+C(6);
C1 = [max(FitSpec)/2,    0,  2.5, max(FitSpec)/2, 3.5,       0]; % Initial guesses for the fit parameters
lb = [      0.000001, -0.01,    1,       0.000001,   1, -100000]; % Lower bounds for the fit parameters
ub = [           Inf,  0.01,   10,            Inf,  10,  100000]; % Upper bounds for the fit parameters 
...
title(['Spectrum ' num2str(Viewspec1) ':, ChiSq ' num2str(RChiSq(Viewspec1)) ', Median ' num2str(MRChiSq) ', Range ' num2str(RanRChiSq)]);
...
title(['Spectrum ' num2str(Viewspec2) ': ChiSq ' num2str(RChiSq(Viewspec2))]);
xlim([EN(Windows(1)-PlotBuffer) EN(Windows(4)+PlotBuffer)]);
ylim([min(fitwin) max(fitwin)]);
ax = gca; ax.YAxis.Exponent = 0;
x1 = EN(Windows(1));
