...
func= @(C,x)C(1)*x.^(-C(2))+C(3);
C1 = [max(Input(m,:))/2,3,1000]; % Initial guesses for the fit parameters
lb=[0.0001, 1, -10000]; % Lower bounds for the fit parameters
ub=[Inf, Inf, 10000]; % Upper bounds for the fit parameters 
...
func= @(C,x)C(1)*x.^(-C(2))+C(3);
C1 = [max(FitSpec)/2,3,0]; % Initial guesses for the fit parameters
lb=[0.00001, 1, -10000]; % Lower bounds for the fit parameters
ub=[Inf, Inf, 10000]; % Upper bounds for the fit parameters ft = lsqcurvefit(func,C1,fitEN,fitwin, lb, ub, options);
ft = lsqcurvefit(func,C1,fitEN,fitwin, lb, ub, options); % Fit Curve to data.
hold on;
